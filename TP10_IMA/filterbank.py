...
class Filterbank():
    ...
    def _valid_bands(self, fs, bands, bandsize):
        ...
        bands_safe = []
        for band in bands:
            if band*np.power(2, bandsize/2) < int(fs/2):
                bands_safe.append(band)
        fc = [band*np.power(2, bandsize/2) for band in bands_safe] # upper cut freq for each band
        assert fc[-1] < (fs/2)
        self.fc = fc
        self.fs = fs
    ...
    def apply(self, signal):
        ...
        return filtered_bands, self.bands
